...
SerialUtil 是一款基于现代技术栈（Rust + Tauri + React）构建的下一代跨平台串口调试工具。它不仅仅是一个简单的串口终端，更是一个集成了多个创新特性的强大串口开发平台。
...
### 1. 极致性能与现代体验
**始于颜值，忠于体验。**
SerialUtil 采用了业界领先的 **Shadcn UI** 组件库与 **React** 框架，为您带来前所未有的串口调试体验。
...
**用你最熟悉的语言处理数据。**
秉承 **Unix 管道哲学**，将 "Do one thing and do it well" 发挥到极致。SerialUtil 不再束缚于内置脚本引擎的贫瘠生态，而是通过**标准流（Stdio）**接口与整个操作系统无缝融合。
...
**解决"端口被占用"的痛点。**
SerialUtil 内置了基于 com0com 的虚拟化技术，实现了**单物理串口的多应用分发**。
*   **一拖多模式**：允许其他串口软件（如串口绘图工具、旧版上位机）同时连接到同一个物理串口，与 SerialUtil 并行工作。
...
### 开启端口共享
1.  确保已安装 com0com。
2.  在 SerialUtil 中连接物理串口（如 COM3）。
3.  点击工具栏的 **"启用共享"** (Enable Sharing)。
4.  SerialUtil 会自动创建一个虚拟端口（如 COM10），其他软件连接 COM10 即可获得 COM3 的数据副本。

### 配置脚本钩子
1.  编写你的脚本（例如 processor.py），确保它从 stdin 读取二进制数据，处理后写入 stdout。
2.  在 SerialUtil 设置中填入执行命令：`python /path/to/processor.py`。
